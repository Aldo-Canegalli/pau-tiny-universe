...
import { ShoppingBag, MessageCircle, ZoomIn } from "lucide-react";
...
export default function ProductCard({ prod, onViewDetails, onQuote }) {
  ...
  const handleImageClick = (e) => {
    // Evitar que se active el click del card padre
    e.stopPropagation();
    if (prod.image_url) {
    ...
    <>
      ...
      >
        ...
        <div
          ...
          className={`w-full h-52 bg-pink-50/50 rounded-2xl mb-4 border-2 border-dashed border-pink-200 flex flex-col items-center justify-center text-pink-300 overflow-hidden relative group ${
        ...
        >
          {prod.image_url ? (
            <>
              ...
              />
              {/* Overlay con ícono de zoom al hacer hover */}
        ...
        <span className="text-xs font-bold text-pink-400 mb-3 uppercase tracking-wider">
          {categoriaNombre}
        </span>
